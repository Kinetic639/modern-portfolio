...
export const Bootcamps = () => {
  ...
  return (
    ...
    >
      <div
        className={`${styles.menu} ${showDesktopMenu ? styles.menuDesktop : ""}`}
      >
        ...
        <div
          className={`${styles.indicator} ${showDesktopMenu ? styles.indicatorDesktop : ""}`}
          style={showDesktopMenu ? indicatorVertical : indicatorHorizontal}
        ></div>
      </div>
      {activeLink < bootcampsList.length ? (
        ...
        >
          <AnimatePresence mode="wait">
            ...
            >
              <p>
                {bootcampsList[activeLink].title}{" "}
                <a
                  className={styles.link}
                  href={bootcampsList[activeLink].link}
                  target="_blank"
                  rel="noreferrer"
                >
                  @ {bootcampsList[activeLink].name}
                </a>
              </p>

              <p className={styles.subHeader}>
                {bootcampsList[activeLink].date}
              </p>
            </motion.div>
          </AnimatePresence>
          <AnimatePresence mode="wait">
            ...
            >
              {t(
                `sections.about.bootcamps.${bootcampsList[activeLink].translation}`,
              )}
            </motion.p>
          </AnimatePresence>
        </motion.div>
        ...
        <AnimatePresence mode="wait">
          <motion.div
            ...
            exit={{ opacity: 0 }}
          >
            <p>{t("sections.about.bootcamps.others")}</p>
            <ul className={styles.techstackColumn}>
              <li className={styles.duty}>
                <VscDebugBreakpointFunction className={styles.dutyPoint} /> Brad
                Traversy
              </li>
              <li className={styles.duty}>
              ...
              <li className={styles.duty}>
                <VscDebugBreakpointFunction className={styles.dutyPoint} />{" "}
                Bartłomiej Borowczyk (Samuraj Programowania)
              </li>
              <li className={styles.duty}>
                <VscDebugBreakpointFunction className={styles.dutyPoint} />
              ...
              <li className={styles.duty}>
                <VscDebugBreakpointFunction className={styles.dutyPoint} /> Adam
                Romański (hello roman)
              </li>
            </ul>
          </motion.div>
        </AnimatePresence>
